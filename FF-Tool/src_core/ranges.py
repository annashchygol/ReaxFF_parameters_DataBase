...
def gatherAllDataByBlock(dataBase, block):
    if (block != 'general'):
        distinctItems = []
        for elem in dataBase:
            print(elem)
            distinctItems = addDistinctItem('', elem[block], distinctItems) #<- either empty first argument; or another function. 



def gatherDataByKey(dataBase, block, directItemKey, quartileFlag = False, randGuess = False):
    """
    A function to gather the data(parameter values) from the dataBase for the particular block per key. 
    """

    #######################
    # Print entire DataBase ti understand how to operate with it.
    #all_hydrogen_params_in_block = list(dataBase[0]['hydrogen'].values())

    #print ("DataBase: >> hydrogen", all_hydrogen_params_in_block, all_hydrogen_params_in_block[0], '\n',)
    #######################
    ...
    if (block != 'general'):
        ...
        print('Gathered Item List size:', len(distinctItems), distinctItems)
...
def getRanges(inFF, dataBase, quartileFlag = False):
    ...
    for block in inFF: #These keys I'm going to use for DB search
        ...
        if (block != None):
            ...
            # If flag ignoreKeys is on, we are ignoring Keys and gathering all the DataByBlock  
